...
- Mode toggles for Fraction/Decimal, π formatting (only for values nearly an integer multiple of π), and Trigonometric (Degrees/Radians).
- Recognizes "x" as multiplication, and absolute value notation using |...|.
- History panel on the right with scrollable, full-width entries (operation and result in different fonts), with individual delete buttons and a Clear History button.
- Buttons arranged in 8 full rows (6 buttons per row) and a bottom panel showing current modes.
- Specific replacements:
    • In Row 4, Column 5: "Ans" replaced with φ (totient) button.
    • In Row 7, Column 4: "exp" button replaced by a "prime#" button (prime count).
    • In Row 7, Column 5: "log10" replaced by a cubic root button (∛) that inserts "cbrt(".
    • In Row 8, Column 3: "Deg/Rad" toggle to change between degree and radian mode (in sync with bottom-right panel).
    • Floor and ceiling buttons in Row 8 directly apply floor/ceil rounding.
    • The factorial button ("!") in Row 8 inserts "!" so that patterns like "123!" are converted to factorial(123).
- The safe evaluation function pre-processes the expression accordingly.
- All buttons are wider, and the overall window and entry are widened.
"""
...
# pi_mode: True means use π formatting (only for numbers nearly an integer multiple of π); False means show exact.
trig_mode = False
fraction_mode = False
pi_mode = True
...
def safe_eval(expression):
    ...
    Evaluate the expression safely after preprocessing:
      ...
      - Convert patterns like "123!" to "factorial(123)"
      - Convert |...| to abs(...)
    """
...
def convert_to_scientific(num):
    """
    Convert a number to scientific notation in the format:
    <mantissa> x 10^<exponent> (6 significant figures)
    """
    try:
        s = format(num, ".6e")
...
def convert_to_pi_format(result):
    """
    If result is a float and nearly equal to an integer multiple of π (within tolerance),
    return a string like "π", "2π", etc.; otherwise, return the exact result as a string.
    """
    if isinstance(result, float):
        factor = round(result / math.pi)
        if math.isclose(result, factor * math.pi, rel_tol=1e-5, abs_tol=1e-5):
            if factor == 1:
                return "π"
            elif factor == -1:
                return "-π"
...
# Additional conversion functions for new buttons:
def insert_pi():
...
def add_history(operation, solution):
    ...
    with the operation on top (centered, Courier New 16 bold) and the solution below (centered, Courier New 16 italic),
    plus a delete button.
    """
    hist_frame = tk.Frame(history_frame, bd=1, relief="solid", padx=5, pady=5)
    op_label = tk.Label(hist_frame, text=operation, font=("Courier New", 16, "bold"), anchor="center", justify="center")
    sol_label = tk.Label(hist_frame, text=solution, font=("Courier New", 16, "italic"), fg="blue", anchor="center", justify="center")
    del_btn = tk.Button(hist_frame, text="X", font=("Courier New", 12), command=lambda: hist_frame.destroy())
    op_label.grid(row=0, column=0, sticky="nsew")
    sol_label.grid(row=1, column=0, sticky="nsew")
    del_btn.grid(row=0, column=1, rowspan=2, padx=5)
...
button_specs = [
    ...
    # Row 5: Trigonometric Functions: sin, cos, tan, asin, acos, atan
    ...
    # Row 6: Advanced Functions: log, ln, Prime, x², 1/x, Frac/Dec
    ...
    # Row 7: Additional Operations: π, Rad→Deg, Deg→Rad, prime#, ∛, mod
    (7, 0, 'π', insert_pi),
...
# Contains: Show π (toggle), !, abs, Deg/Rad (toggle), floor, ceil.
new_button_specs = [
